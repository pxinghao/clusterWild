...
while (1==1) {
    ...
    // System.  out.println(s"$numNewCenters\t")

    // numNewCenters = clusterGraph.vertices.filter(v => (v._2 == -100) && (scala.util.Random.nextFloat < epsilon/maxDeg.toFloat)).count
    // System.out.println(s"$numNewCenters\t")
    // clusterGraph = clusterGraph.joinVertices( clusterGraph.vertices.filter(v => (v._2 == -100) && (scala.util.Random.nextFloat < epsilon/maxDeg.toFloat)) )((vId, attr, active) => centerID)
    // clusterGraph = clusterGraph.joinVertices( clusterGraph.vertices.filter(v => (v._2 > scala.util.Random.nextFloat*n ))
    clusterGraph = clusterGraph.joinVertices(clusterGraph.vertices.filter(v => v._2 == -100).sample(false, math.min(epsilon / maxDeg, 1), scala.util.Random.nextInt(1000)))((vId, attr, active) => centerID).cache()
    // clusterGraph.cache()
